const TIPS = {
  ...
  "导游": `——有坏人在在追你？放心，我设法会拖住他们的！
    ...
    <安全检查.限定>:游戏日夜晚，你可以选择一个地点，并获得该地点所有玩家的名单。当你是凶手时，此技能为普通技能、不再是限定技。
  `,
  ...
  "侦探": `[推理]
    ...
    ②<普通>：你发动嗜血如命后，你可以在任意地点，留下一条<痕迹>类额外线索。
  ...
  "神秘人": `[善战] 
    ...
    技能：
    <轻车熟路>： 
    ①<锁定>当时你选择指定死者的谋杀方式时，这次谋杀必定成功。之后，你获知因这次谋杀产生的所有额外线索的名称和对应数量，并可以将之全部清除。  
    ②<普通>当你发现凶案时，你可以得知你得到的是犯罪信息还是诡计。

    PS：若你需要遗留自己的线索作为额外线索（如杀死[善战]角色、点杀卧室内角色），你可以将其清除，但仍需消耗自己的一条线索。<轻车熟路①>
  `,

  "程序员": `——别动我，让我改完这个Bug！
    杀人手法：刀杀|钝击
    拥有线索：（痕迹：男人的手掌印）（痕迹：男人的脚印）（遗留物：手机）（遗留物：智能手机）（遗留物：手表）（遗留物：零食）（遗留物：眼镜）

    技能：
    <加班.普通>：游戏日夜晚，获知你过夜地点的人员名单。你不能连续两晚发动该技能。
    <过劳.锁定>：若你发动过<加班>，且在<嗜血如命>时你死亡的地点只有你一位死者，犯罪信息中的死因变为<猝死>。
    <信念坚定.锁定>：当你所处的地点人数上限不大于3时，除非你主动移动，你或你的尸体位置均不会被改变。
  `,
